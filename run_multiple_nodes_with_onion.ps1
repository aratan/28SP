# Script para ejecutar múltiples nodos con enrutamiento cebolla real
...
for ($i = 1; $i -le 5; $i++) {
    ...
    Start-Process -FilePath ".\p2p-app.exe" -ArgumentList "-listen=/ip4/0.0.0.0/tcp/$p2pPort -port=$port -node-id=node$i" -WorkingDirectory "node$i" -NoNewWindow
}
